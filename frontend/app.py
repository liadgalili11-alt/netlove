...
if not api_client.check_health():
    st.error("⚠️ השרת אינו פעיל. הפעל את FastAPI backend לפני השימוש:\n`uvicorn backend.main:app --reload`")
    st.stop()

st.markdown("---")

# ── Workflow (3-column layout) ────────────────────────────────────────────────
...
st.markdown(
    ...
    "Netlove AI Content Studio © 2024 | Powered by GPT-4o + Meta Graph API + TikTok Content Posting API"
    "</p>",
    unsafe_allow_html=True,
)
